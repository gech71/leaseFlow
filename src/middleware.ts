...
async function handleAuthenticatedSession(
  ...
  if (!AUTH_API_BASE_URL) {
    console.error(
      "Middleware Error: Auth API base URL is not configured for token refresh.",
    );
    return NextResponse.redirect(new URL(LOGIN_PATH, request.url));
  }
...
export async function middleware(request: NextRequest) {
  ...
  const publicPaths = [LOGIN_PATH, "/portal/connect", "/portal/billing"];
  ...
  if (pathname === "/") {
    if (hasSessionToken) {
      response = NextResponse.redirect(
        new URL(ADMIN_DEFAULT_PATH, request.url),
      );
    } else {
