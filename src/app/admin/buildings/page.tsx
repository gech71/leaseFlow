...
"use client"; // Keep "use client" if client-side hooks like useState, useEffect for dialogs are essential.
              // However, for data display, it could be a Server Component fetching data directly.
              // Given the existing structure with useState for dialogs, we'll keep it client for now and pass fetched data as props.
              // Let's change this to make the page a Server Component and handle dialogs with client components if necessary.

import { useState, useEffect } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation'; // Will be used if we keep client-side interactions
...
import type { Building as BuildingTypePrisma, PenaltyTier as PenaltyTierTypePrisma } from '@prisma/client'; // Use Prisma's generated types
...
import { databaseService } from '@/lib/services/databaseService'; // Import the service


// Define the expected props for the client component part if we split
...
interface BuildingWithPenaltyTiers extends BuildingTypePrisma {
...
export default async function BuildingsPage() {
  ...
  // Data fetched on the server is directly available here.
  // For client-side interactivity like the delete dialog, we need a client component.
  // We'll create a wrapper client component to handle state for the dialog.
  
  return <BuildingsClientPage initialBuildings={buildingsData} />;
}


// New Client Component to handle client-side state and interactions
function BuildingsClientPage({ initialBuildings }: { initialBuildings: BuildingWithPenaltyTiers[] }) {
  ...
  // Update local state if initialBuildings prop changes (e.g., after server action revalidation)
  useEffect(() => {
  ...
  const handleDeleteBuilding = async () => {
    ...
    if (result.success) {
      ...
      // Optimistically update UI or rely on router.refresh() / revalidatePath from action
      setBuildings(prev => prev.filter(b => b.id !== buildingToDelete.id));
      // router.refresh(); // This can also be used to refetch data on the server
    } else {
