...
export default function BuildingsPage() {
  ...
  const handleAddUIPenaltyRule = () => {
    const newRule: UIPenaltyRule = { 
      ...
      scope: 'Building', 
      feeType: 'Fixed',
      feeValue: 0, // Default fee value
      durationDays: undefined, // Default duration
    };
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Group UI rules by their effective scope key (e.g., "Building", "Floor_1st Floor", "SpecificSpaces_Unit A,Unit B")
    ...
    currentBuildingForm.uiPenaltyRules.forEach(uiRule => {
      if (!uiRule.feeType || uiRule.feeValue === undefined || uiRule.feeValue < 0) {
        ...
        throw new Error("Incomplete UI rule."); // Stop processing
      }
    ...
    // Process each group to calculate fromDay and toDay
    for (const scopeKey in groupedUIRules) {
      const rulesInScope = groupedUIRules[scopeKey]; // Order is preserved from UI
      ...
      for (let i = 0; i < rulesInScope.length; i++) {
        ...
        // If it's the last rule in this scope's sequence OR durationDays is not set/invalid, it's indefinite
        ...
        finalPenaltyTiers.push({
          ...
          feeValue: Number(uiRule.feeValue!), // Already validated feeValue exists
          scope: uiRule.scope,
        ...
        if (toDay !== null) {
          ...
          break; // This was the last (indefinite) rule for this scope
        }
      }
    }
  ...
  const openAddForm = () => {
    ...
    setCurrentBuildingForm({ 
      name: '', 
      address: '', 
      uiPenaltyRules: [] // Start with no rules, user adds them
    });
    setIsFormOpen(true);
  };
  ...
  const openEditForm = (building: Building) => {
    ...
    // Group loaded PenaltyTiers by their scope key to reconstruct UI rules
    ...
    // For each scope group, convert sorted PenaltyTiers back to UIPenaltyRules
    Object.values(tiersByScopeKey).forEach(scopeGroup => {
      scopeGroup.sort((a,b) => a.fromDay - b.fromDay).forEach((tier, index) => {
        ...
        // Else, if toDay is null, durationDays can be left undefined for the UI (signifying "indefinite" for the last rule in sequence)
  ...
  return (
    <div className="animate-fadeIn">
      <PageHeader
        ...
        icon={BuildingIcon}
        description="Add/edit buildings. Define late fee policies: add rules, specify duration, fee, and scope (Building, Floor, or Space(s)). Last rule per scope is indefinite."
        actions={
      ...
      <Dialog open={isFormOpen} onOpenChange={(isOpen) => {
        ...
        <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            ...
            <DialogDescription>
              Fill in building details. Penalty rules are sequential within each scope. Leave Duration blank for the last rule in a scope to make it indefinite.
            </DialogDescription>
          </DialogHeader>
          
          <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-1 space-y-4"> {/* Scrollable form area */}
                  ...
                  <CardContent className="p-2 space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">Leave blank for last rule in scope (indefinite).</p>
                        </div>
                        <div>
            ...
            <DialogFooter className="pt-4 border-t flex-shrink-0 p-4"> 
              <DialogClose asChild>
