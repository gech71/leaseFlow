...
import { updateUserAssignments, updateUserDetailsAction } from './actions';
...
export function UserManagementClientPage({
  ...
  const handleEditUserDetails = (user: ClientUserWithAssignments) => {
    if (!canManageUserAssignments) {
        toast({ title: "Permission Denied", description: "You do not have permission to edit user details.", variant: "destructive" });
        return;
    }
    setCurrentUserToEdit(user);
  ...
  const handleSaveUserDetails = async (values: UserDetailsFormValues) => {
    if (!currentUserToEdit) return;
    setIsSaving(true);
    const result = await updateUserDetailsAction(currentUserToEdit.id, values);
    setIsSaving(false);
  ...
  return (
    ...
    <Card className="shadow-lg">
                  ...
                  {paginatedUsers.map((user) => (
                    <TableRow key={user.id}>
                      ...
                      <TableCell className="text-right space-x-1">
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleEditAssignments(user)} disabled={isSaving || !canViewUserManagement}>
                              <Edit className="h-4 w-4" />
                            </Button>
                          </TooltipTrigger>
                          <TooltipContent><p>Edit Assignments</p></TooltipContent>
                        </Tooltip>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      {currentUserToEdit && (
        <>
          <Dialog open={isAssignmentsDialogOpen} onOpenChange={(open) => { if (!open) setCurrentUserToEdit(null); setIsAssignmentsDialogOpen(open); }}>
            <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
              ...
              <DialogFooter className="pt-4 border-t mt-auto">
                ...
                {canManageUserAssignments && (
                  <Button onClick={handleSaveAssignments} disabled={isSaving} className="bg-primary hover:bg-primary/90 text-primary-foreground">
          ...
          <Dialog open={isUserDetailsDialogOpen} onOpenChange={(open) => { if (!open) setCurrentUserToEdit(null); setIsUserDetailsDialogOpen(open); }}>
            <DialogContent>
                <DialogHeader>
                ...
                <Form {...userDetailsForm}>
                    <form onSubmit={userDetailsForm.handleSubmit(handleSaveUserDetails)} className="space-y-4">
