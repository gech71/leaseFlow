...
import { Banknote, FileText, User, AlertTriangle, CheckCircle, Loader2, Edit, Trash2, Zap, CreditCard, CalendarIcon as CalendarLucideIcon, InfoIcon, Building as BuildingIconLucide, UploadCloud, MessageSquare, ShieldCheck, ShieldX, Paperclip, Eye, EyeOff, Search } from 'lucide-react';
...
export function BillingClientPage({ initialData }: BillingClientPageProps) {
  ...
  const filteredAgreementsForGeneration = useMemo(() => {
    ...
    return agreements.filter(agreement => {
      ...
      if (generationFilterStatus === 'ready' && !isDueForGeneration) return false;
      if (generationFilterStatus === 'upcoming' && isDueForGeneration) return false;
  ...
  const generationTotalPages = Math.ceil(filteredAgreementsForGeneration.length / generationItemsPerPage);
  const paginatedAgreementsForGeneration = filteredAgreementsForGeneration.slice(
 ...
 useEffect(() => {
  ...
  return (
    <div className="animate-fadeIn">
      <PageHeader title="Billing Management" icon={Banknote} description="Generate and manage bills. Verify tenant-submitted payments." />
      ...
      {canGenerateBills && (
        <Card className="mb-6 shadow-sm">
          ...
          <CardContent>
            <Button onClick={handleGenerateAllDueBills} className="w-full md:w-auto bg-accent text-accent-foreground hover:bg-accent/90" disabled={isLoading || agreements.length === 0}>
          ...
          <div className="px-6 pb-6 pt-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-4 mb-6">
              ...
              <Select value={generationFilterStatus} onValueChange={(value) => setGenerationFilterStatus(value as any)}>
          ...
          <DialogContent className="sm:max-w-md">
              ...
              <Form {...paymentForm}>
                  <form onSubmit={paymentForm.handleSubmit(handleRecordPaymentSubmit)} className="space-y-4 py-2">
                      <FormField control={paymentForm.control} name="paymentDate" render={({ field }) => ( <FormItem className="flex flex-col"><FormLabel>Payment Date<span className="text-destructive ml-1">*</span></FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant="outline" className={`w-full pl-3 text-left font-normal ${!field.value && "text-muted-foreground"}`} disabled={isReadOnly || isLoading || !canManagePayments}>{field.value ? format(field.value, "PPP") : <span>Pick payment date</span>}<CalendarLucideIcon className="ml-auto h-4 w-4 opacity-50" /></Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={(date) => date > new Date() || date < new Date("1900-01-01")} initialFocus /></PopoverContent></Popover><FormMessage /></FormItem>)} />
                      <FormField control={paymentForm.control} name="paymentMethod" render={({ field }) => ( <FormItem><FormLabel>Payment Method<span className="text-destructive ml-1">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isReadOnly || isLoading || !canManagePayments}><FormControl><SelectTrigger><SelectValue placeholder="Select payment method" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Card">Card</SelectItem><SelectItem value="Cash">Cash</SelectItem><SelectItem value="Bank Transfer">Bank Transfer</SelectItem><SelectItem value="Wallet">Wallet</SelectItem><SelectItem value="Check">Check</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
                      {(paymentMethodWatcher === "Bank Transfer" || paymentMethodWatcher === "Wallet") && ( <FormField control={paymentForm.control} name="bankOrWalletName" render={({ field }) => ( <FormItem><FormLabel>{paymentMethodWatcher === "Bank Transfer" ? "Bank Name" : "Wallet Name"}<span className="text-destructive ml-1">*</span></FormLabel><FormControl><Input placeholder={`Enter ${paymentMethodWatcher === "Bank Transfer" ? "bank name" : "wallet name"}`} {...field} disabled={isReadOnly || isLoading || !canManagePayments}/></FormControl><FormMessage /></FormItem>)} /> )}
                      <FormField control={paymentForm.control} name="paymentReference" render={({ field }) => ( <FormItem><FormLabel>Payment Reference (Optional)</FormLabel><FormControl><Input placeholder="Transaction ID, Check No., etc." {...field} disabled={isReadOnly || isLoading || !canManagePayments}/></FormControl><FormMessage /></FormItem>)} />
                      {canManagePayments && <div> <Label htmlFor="adminPaymentProofFile" className="flex items-center mb-1 text-sm font-medium"> <Paperclip className="mr-2 h-4 w-4 text-primary" /> Attach Payment Slip </Label> <Input id="adminPaymentProofFile" type="file" ref={adminProofFileInputRef} onChange={handleAdminFileSelect} className="text-sm file:mr-2 file:py-1.5 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" disabled={isReadOnly || isLoading}/> {adminSelectedProofFile && <p className="text-xs text-muted-foreground mt-1">Selected: {adminSelectedProofFile.name}</p>} </div>}
                      <FormField control={paymentForm.control} name="adminVerificationNotes" render={({ field }) => ( <FormItem><FormLabel>Admin Notes (Optional)</FormLabel><FormControl><Textarea placeholder="Verification notes..." rows={2} {...field} disabled={isReadOnly || isLoading || !canManagePayments}/></FormControl><FormMessage /></FormItem>)} />
      ...
      <Dialog open={isVerificationDialogOpen} onOpenChange={(isOpen) => { setIsVerificationDialogOpen(isOpen); if (!isOpen) { setBillForVerification(null); paymentForm.reset(); setAdminSelectedProofFile(null); if(adminProofFileInputRef.current) adminProofFileInputRef.current.value = ""; }}}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="flex-shrink-0 border-b pb-4">
              {billForVerification && ( <div className="text-sm space-y-2 py-2"> <p><strong>Tenant Notes:</strong> {billForVerification.tenantPaymentNotes || <span className="italic text-muted-foreground">No notes provided.</span>}</p> <p><strong>Submitted Proof:</strong> {billForVerification.paymentProofUrl ? <Button asChild variant="link" size="sm" className="p-0 h-auto"><a href={billForVerification.paymentProofUrl} target="_blank" rel="noopener noreferrer">{billForVerification.paymentProofUrl}</a></Button> : <span className="italic text-muted-foreground">No proof URL found.</span>} </p> </div> )}
            </div>

            <div className="flex-grow overflow-y-auto pr-4 py-2">
              <Form {...paymentForm}>
                  <form className="space-y-4"> 
                      <FormField control={paymentForm.control} name="paymentDate" render={({ field }) => ( <FormItem className="flex flex-col"><FormLabel>Actual Payment Date<span className="text-destructive ml-1">*</span></FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant="outline" className={`w-full pl-3 text-left font-normal ${!field.value && "text-muted-foreground"}`} disabled={isLoading || !canManagePayments}>{field.value ? format(field.value, "PPP") : <span>Pick payment date</span>}<CalendarLucideIcon className="ml-auto h-4 w-4 opacity-50" /></Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={(date) => date > new Date() || date < new Date("1900-01-01")} initialFocus /></PopoverContent></Popover><FormMessage /></FormItem>)} />
                      <FormField control={paymentForm.control} name="paymentMethod" render={({ field }) => ( <FormItem><FormLabel>Actual Payment Method<span className="text-destructive ml-1">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isLoading || !canManagePayments}><FormControl><SelectTrigger><SelectValue placeholder="Select payment method" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Card">Card</SelectItem><SelectItem value="Cash">Cash</SelectItem><SelectItem value="Bank Transfer">Bank Transfer</SelectItem><SelectItem value="Wallet">Wallet</SelectItem><SelectItem value="Check">Check</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
                      {(paymentMethodWatcher === "Bank Transfer" || paymentMethodWatcher === "Wallet") && ( <FormField control={paymentForm.control} name="bankOrWalletName" render={({ field }) => ( <FormItem><FormLabel>{paymentMethodWatcher === "Bank Transfer" ? "Bank Name" : "Wallet Name"}<span className="text-destructive ml-1">*</span></FormLabel><FormControl><Input placeholder={`Enter ${paymentMethodWatcher === "Bank Transfer" ? "bank name" : "wallet name"}`} {...field} disabled={isLoading || !canManagePayments}/></FormControl><FormMessage /></FormItem>)} /> )}
                      <FormField control={paymentForm.control} name="paymentReference" render={({ field }) => ( <FormItem><FormLabel>Actual Payment Reference</FormLabel><FormControl><Input placeholder="Transaction ID, Check No., etc." {...field} disabled={isLoading || !canManagePayments}/></FormControl><FormMessage /></FormItem>)} />
                      {canManagePayments && <div className="space-y-2"> <Label htmlFor="adminVerificationProofFile" className="flex items-center text-sm font-medium"> <Paperclip className="mr-2 h-4 w-4 text-primary" /> Replace/Add Payment Slip </Label> <Input id="adminVerificationProofFile" type="file" ref={adminProofFileInputRef} onChange={handleAdminFileSelect} className="text-sm file:mr-2 file:py-1.5 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" disabled={isLoading}/> {adminSelectedProofFile && <p className="text-xs text-muted-foreground mt-1">New file selected: {adminSelectedProofFile.name}</p>} </div>}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleEditSubmit)} className="space-y-4 py-2">
              ...
              <div>
                  <Label htmlFor="adminEditProofFile" className="flex items-center mb-1 text-sm font-medium"> <Paperclip className="mr-2 h-4 w-4 text-primary" /> Attach/Replace Payment Slip </Label>
              ...
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="w-[20%]">Tenant / Space</TableHead>
                        <TableHead className="w-[15%] hidden md:table-cell">Dates</TableHead>
                        <TableHead className="w-[25%] text-right">Amount (Birr)</TableHead>
                      ...
                      {paginatedBills.map((bill) => (
                        <TableRow key={bill.id} className={`${bill.currentStatus === 'Overdue' ? 'bg-destructive/5 hover:bg-destructive/10' : ''} ${bill.currentStatus === 'PendingVerification' ? 'bg-blue-500/5 hover:bg-blue-500/10' : ''}`}>
                          <TableCell className="font-medium">
                            ...
                            <div className="text-xs text-muted-foreground">{bill.agreement?.space?.spaceIdName}</div>
                          </TableCell>
                          <TableCell className="hidden md:table-cell text-sm">
                            <div>Bill: {format(parseISO(bill.billDate), 'dd-MMM-yy')}</div>
                            <div className={`text-xs ${bill.currentStatus === 'Overdue' ? 'text-destructive font-semibold' : 'text-muted-foreground'}`}>
                              Due: {format(parseISO(bill.dueDate), 'dd-MMM-yy')}
                            </div>
                          </TableCell>
                          <TableCell className="text-right text-xs">
                             <div className="font-semibold text-sm text-foreground">{bill.totalAmount.toFixed(2)} Birr</div>
                            <div className="text-muted-foreground">Rent: {bill.rentAmount.toFixed(2)} Birr</div>
                            <div className="text-muted-foreground">Utility: {bill.utilityBreakdown.reduce((s, u) => s + u.amount, 0).toFixed(2)} Birr</div>
                            {bill.penaltyAmount ? <div className="text-destructive font-medium">Penalty: {bill.penaltyAmount.toFixed(2)} Birr</div> : null}
                          </TableCell>
                            ...
                            <div className="flex flex-col items-end gap-1 sm:flex-row sm:items-center sm:justify-end sm:gap-0">
                              {bill.currentStatus === 'Paid' ? (
                                <>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button variant="ghost" size="icon" onClick={() => handleOpenPaymentDialog(bill)} className="h-8 w-8" disabled={isLoading}>
                                ...
                                <>
                                  {canManagePayments && (
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <Button variant="ghost" size="icon" onClick={() => handleOpenVerificationDialog(bill)} className="h-8 w-8 text-blue-600" disabled={isLoading}>
                                          <ShieldCheck className="h-4 w-4" />
                                          <span className="sr-only">Verify Payment</span>
                                        </Button>
                                      </TooltipTrigger>
                                      <TooltipContent><p>Verify Payment</p></TooltipContent>
                                    </Tooltip>
                                  )}
                                </>
                              ) : ( // Pending or Overdue
                              ...
                              {canDeleteBills && bill.currentStatus !== 'Paid' && (
                                 <Tooltip>
                                  ...
                                  <TooltipContent><p>Delete Bill</p></TooltipContent>
                                </Tooltip>
                              )}
                            </div>
                          </TableCell>
