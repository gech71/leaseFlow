...
import {
  ...
  type UtilityBreakdownItem as UtilityBreakdownItemPrismaOriginal,
...
/**
 * Clamp a date to the agreement billing day in UTC for the given date's year/month.
 * Ensures generated bills keep the same day-of-month (or the month's last day when shorter).
 */
function clampToBillingDayUTC(date: Date, billingDay: number): Date {
  const year = date.getUTCFullYear();
  const month = date.getUTCMonth();
  // days in month: construct the 0th day of next month in UTC
  const daysInMonth = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
  const day = Math.min(Math.max(1, billingDay), daysInMonth);
  return new Date(Date.UTC(year, month, day));
}

/**
 * Add months in UTC while preserving billing day semantics (clamped to month length).
 */
function addMonthsKeepBillingDayUTC(
  date: Date,
  monthsToAdd: number,
  billingDay: number,
): Date {
  ...
  const year = Math.floor(targetMonth / 12) + targetYear;
  const month = ((targetMonth % 12) + 12) % 12; // normalize
  const daysInTargetMonth = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
  const day = Math.min(Math.max(1, billingDay), daysInTargetMonth);
  return new Date(Date.UTC(year, month, day));
}
...
export async function getBillingPageDataAction(): Promise<SerializedBillingPageData> {
  ...
  const [
    ...
    databaseService.getAllBuildings({
      where: buildingWhere,
      include: { penaltyPolicyTiers: true, spaces: true },
    }),
  ...
  const serializedAgreements = agreementsData.map((ag) => ({
    ...(ag as AgreementPrismaOriginal & {
      tenant: TenantPrismaOriginal | null;
      space:
        | (SpacePrismaOriginal & {
            building:
              | (BuildingPrismaOriginal & {
                  penaltyPolicyTiers: PenaltyTierPrismaOriginal[];
                  spaces: SpacePrismaOriginal[];
                })
              | null;
          })
        | null;
    ...
    nextPaymentDueDate: ag.nextPaymentDueDate
      ? ag.nextPaymentDueDate.toISOString()
      : EPOCH_ISO_STRING,
    ...
    tenant: ag.tenant
      ? {
          ...
          createdAt: ag.tenant.createdAt
            ? ag.tenant.createdAt.toISOString()
            : EPOCH_ISO_STRING,
          updatedAt: ag.tenant.updatedAt
    ...
    space: ag.space
      ? {
          ...(ag.space as SpacePrismaOriginal & {
            building:
              | (BuildingPrismaOriginal & {
                  penaltyPolicyTiers: PenaltyTierPrismaOriginal[];
                  spaces: SpacePrismaOriginal[];
          ...
          createdAt: ag.space.createdAt
            ? ag.space.createdAt.toISOString()
            : EPOCH_ISO_STRING,
          updatedAt: ag.space.updatedAt
          ...
          building: ag.space.building
            ? {
                ...(ag.space.building as BuildingPrismaOriginal & {
                  penaltyPolicyTiers: PenaltyTierPrismaOriginal[];
                  spaces: SpacePrismaOriginal[];
                }),
                createdAt: ag.space.building.createdAt
                  ? ag.space.building.createdAt.toISOString()
                  : EPOCH_ISO_STRING,
                updatedAt: ag.space.building.updatedAt
                  ? ag.space.building.updatedAt.toISOString()
                  : ag.space.building.createdAt
                  ? ag.space.building.createdAt.toISOString()
                  : EPOCH_ISO_STRING,
                penaltyPolicyTiers: (
                  ag.space.building.penaltyPolicyTiers || []
                ).map((pt) => ({ ...pt, feeValue: Number(pt.feeValue) })),
  ...
  const serializedSpaces = spacesData.map((s) => ({
    ...
    building: s.building
      ? {
          ...(s.building as BuildingPrismaOriginal & {
            penaltyPolicyTiers: PenaltyTierPrismaOriginal[];
            spaces: SpacePrismaOriginal[];
          }),
          createdAt: s.building.createdAt
            ? s.building.createdAt.toISOString()
            : EPOCH_ISO_STRING,
          updatedAt: s.building.updatedAt
            ? s.building.updatedAt.toISOString()
            : s.building.createdAt
            ? s.building.createdAt.toISOString()
            : EPOCH_ISO_STRING,
          penaltyPolicyTiers: (s.building.penaltyPolicyTiers || []).map(
            (pt) => ({ ...pt, feeValue: Number(pt.feeValue) }),
          ),
          spaces: (s.building.spaces || []).map((sp) => ({
            ...
            createdAt: sp.createdAt
              ...
              : EPOCH_ISO_STRING,
            updatedAt:
              sp.updatedAt?.toISOString() ||
              (sp.createdAt ? sp.createdAt.toISOString() : EPOCH_ISO_STRING),
          })),
        }
      : null,
  ...
  const serializedBuildings = buildingsData.map((b) => ({
    ...
    updatedAt: b.updatedAt
      ...
      : EPOCH_ISO_STRING,
    penaltyPolicyTiers: (b.penaltyPolicyTiers || []).map((pt) => ({
      ...pt,
    ...
    spaces: (b.spaces || []).map((s) => ({
      ...
      updatedAt:
        s.updatedAt?.toISOString() ||
        (s.createdAt ? s.createdAt.toISOString() : EPOCH_ISO_STRING),
    })),
  ...
  const serializedBills = billsDataRaw.map((billRaw) => {
    ...
    if (typeof rawUtilityData === "string") {
      try {
        ...
        console.error(
          `Failed to parse utilityBreakdown JSON for bill ${billRaw.id}:`,
          e,
          rawUtilityData,
        );
      }
    ...
    const billCreatedAt = billRaw.createdAt
      ? billRaw.createdAt.toISOString()
      : EPOCH_ISO_STRING;
    const billUpdatedAt = billRaw.updatedAt
      ? billRaw.updatedAt.toISOString()
      : billCreatedAt;
    const agreementForBill = billRaw.agreement as AgreementPrismaOriginal & {
      tenant: TenantPrismaOriginal | null;
      space:
        | (SpacePrismaOriginal & {
            building:
    ...
    return {
      ...
      penaltyAmount: billRaw.penaltyAmount
      ...
      agreement: agreementForBill
        ? {
            ...
            initialPaymentAmount: agreementForBill.initialPaymentAmount
              ? Number(agreementForBill.initialPaymentAmount)
              : null,
            createdAt: agreementForBill.createdAt
              ? agreementForBill.createdAt.toISOString()
              : EPOCH_ISO_STRING,
            updatedAt: agreementForBill.updatedAt
              ? agreementForBill.updatedAt.toISOString()
              : agreementForBill.createdAt
              ? agreementForBill.createdAt.toISOString()
              : EPOCH_ISO_STRING,
            startDate: agreementForBill.startDate
              ? agreementForBill.startDate.toISOString()
              : EPOCH_ISO_STRING,
            nextPaymentDueDate: agreementForBill.nextPaymentDueDate
              ? agreementForBill.nextPaymentDueDate.toISOString()
              : EPOCH_ISO_STRING,
            initialPaymentDate:
              agreementForBill.initialPaymentDate?.toISOString() || null,
            endDate: agreementForBill.endDate?.toISOString() || null,
            tenant: agreementForBill.tenant
              ? {
                  ...agreementForBill.tenant,
                  createdAt: agreementForBill.tenant.createdAt
                    ? agreementForBill.tenant.createdAt.toISOString()
                    : EPOCH_ISO_STRING,
                  updatedAt: agreementForBill.tenant.updatedAt
                    ? agreementForBill.tenant.updatedAt.toISOString()
                    : agreementForBill.tenant.createdAt
                    ? agreementForBill.tenant.createdAt.toISOString()
                    : EPOCH_ISO_STRING,
                }
              : null,
            space: agreementForBill.space
              ? {
                  ...
                  utilityProrationShare: Number(
                    agreementForBill.space.utilityProrationShare,
                  ),
                  monthlyRentalPrice: Number(
                    agreementForBill.space.monthlyRentalPrice,
                  ),
                  createdAt: agreementForBill.space.createdAt
                    ? agreementForBill.space.createdAt.toISOString()
                    : EPOCH_ISO_STRING,
                  updatedAt: agreementForBill.space.updatedAt
                    ? agreementForBill.space.updatedAt.toISOString()
                    : agreementForBill.space.createdAt
                    ? agreementForBill.space.createdAt.toISOString()
                    : EPOCH_ISO_STRING,
                  building: agreementForBill.space.building
                    ? {
                        ...
                        status:
                          agreementForBill.space.building.status || "Active",
                        createdAt: agreementForBill.space.building.createdAt
                          ? agreementForBill.space.building.createdAt.toISOString()
                          : EPOCH_ISO_STRING,
                        updatedAt: agreementForBill.space.building.updatedAt
                          ? agreementForBill.space.building.updatedAt.toISOString()
                          : agreementForBill.space.building.createdAt
                          ? agreementForBill.space.building.createdAt.toISOString()
                          : EPOCH_ISO_STRING,
                        penaltyPolicyTiers: (
                          agreementForBill.space.building.penaltyPolicyTiers ||
                          []
                        ).map((pt) => ({
                          ...pt,
                          feeValue: Number(pt.feeValue),
                        ...
                        spaces: (
                          agreementForBill.space.building.spaces || []
                        ).map((s) => ({
                          ...s,
                          area: Number(s.area),
                          utilityProrationShare: Number(
                            s.utilityProrationShare,
                          ),
                          monthlyRentalPrice: Number(s.monthlyRentalPrice),
                          createdAt: s.createdAt
                            ? s.createdAt.toISOString()
  ...
  const serializedBuildingMonthlyUtilities = buildingMonthlyUtilitiesData.map(
    (bu) => ({
      ...(bu as BuildingMonthlyUtilitiesPrisma & {
      ...
      updatedAt: bu.updatedAt
        ? bu.updatedAt.toISOString()
        : bu.createdAt
        ? bu.createdAt.toISOString()
        : EPOCH_ISO_STRING,
      utilities: (bu.utilities || []).map((u) => ({
        ...u,
...
export async function generateBillAndUpdateAgreementAction(
  ...
  try {
    ...
    // by one month so the monthly bill represents the next month. This
    // prevents the monthly bill from colliding with the aggregated invoice
    // and ensures the UI shows the monthly period after the aggregated
    // prepayment.
    ...
    let effectiveBillDate = targetBillDate;
    // Determine the agreement's billing day in UTC (day of month from the start date).
    // Fallback to the target bill date's day if agreement.startDate is missing.
    const billingDay = agreement.startDate
      ? (agreement.startDate as Date).getUTCDate()
      : targetBillDate.getUTCDate();
    if (prepaidBillsForTarget.length > 0) {
      // Shift by one month but preserve the billing day semantics in UTC
      effectiveBillDate = addMonthsKeepBillingDayUTC(
        targetBillDate,
        1,
        billingDay,
      );
    ...
    // Normalize the final effective bill date to the agreement billing day (clamped to month length)
    effectiveBillDate = clampToBillingDayUTC(effectiveBillDate, billingDay);

    // Use month-level range to avoid creating more than one bill in the same month
    // (handles cases where an aggregated/prepaid invoice exists on a different day
    // in the same month or where a previous monthly bill was created on a different day).
    const effectiveMonthStart = new Date(
      Date.UTC(
        effectiveBillDate.getUTCFullYear(),
        effectiveBillDate.getUTCMonth(),
        1,
      ),
    );
    const effectiveNextMonthStart = addMonthsUTC(effectiveMonthStart, 1);

    const existingBill = await databaseService.getAllBills({
      where: {
        agreementId: agreement.id,
        billDate: { gte: effectiveMonthStart, lt: effectiveNextMonthStart },
        AND: [{ isPrepaid: { not: true } }],
      },
    });

    if (existingBill.length > 0) {
      // If an existing non-prepaid bill exists for this month, check whether
      // all remaining months up to the agreement's end date already have
      // non-prepaid bills. If so, mark agreement expired and free the space.
      if (agreement.endDate) {
        // Start of the agreement end month (inclusive). We'll compute an
        // exclusive upper bound by adding one month so the loop includes the
        // end month itself.
        const agreementEndMonthStart = new Date(
          Date.UTC(
            (agreement.endDate as Date).getUTCFullYear(),
            (agreement.endDate as Date).getUTCMonth(),
            1,
          ),
        );
        const agreementEndMonthExclusive = addMonthsUTC(
          agreementEndMonthStart,
          1,
        );
        ...
        const nextDueMonthStart = new Date(
          Date.UTC(
            (nextDue as Date).getUTCFullYear(),
            (nextDue as Date).getUTCMonth(),
            1,
          ),
        );

        // Iterate months from nextDueMonthStart (inclusive) to agreementEndMonthStart (exclusive)
        let monthCursor = nextDueMonthStart;
        let allCovered = true;
        while (monthCursor < agreementEndMonthExclusive) {
          const monthStart = monthCursor;
          const monthEnd = addMonthsUTC(monthStart, 1);
          const billsForMonth = await databaseService.getAllBills({
            where: {
              agreementId: agreement.id,
        ...
        if (allCovered) {
          // Mark agreement expired and free space/tenant link
          if (
          ...
          return {
            success: false,
            error: `Agreement term completed; agreement marked Expired and no further bills can be generated.`,
          };
        }
      }
      ...
      return {
        success: false,
        error: `A bill for ${format(effectiveBillDate, "MMMM yyyy")} for ${
    ...
    // aggregated invoice exists) for month-based calculations and utilities.
    let rentAmount = agreement.monthlyRentalPrice; // Assume full rent by default

    // Calculate how many months have passed from the agreement start date to the effective bill date.
    // This correctly handles cases across year boundaries.
    const monthsPassed = differenceInCalendarMonths(
      effectiveBillDate,
      agreement.startDate,
    );

    // If the number of months passed is less than the number of months paid upfront,
    ...
    const isWithinPrepaid =
      ...
      monthsPassed < agreement.initialPaymentMonths;
    if (isWithinPrepaid) {
    ...
    const dueDate = effectiveBillDate;
    ...
    const billCreateInput: Prisma.BillCreateInput = {
      ...
      status:
        isBefore(targetBillDate, today) && initialPenalty > 0
          ? "Overdue"
          : "Pending",
    ...
    const newNextDue = addMonthsKeepBillingDayUTC(
      effectiveBillDate,
      1,
      billingDay,
    );
    ...
    const serializedBill = {
      ...
      penaltyAmount: newBill.penaltyAmount
        ? Number(newBill.penaltyAmount)
        : null,
...
export async function recordPaymentOrVerificationAction(
  ...
  try {
    ...
    if (
      (isBefore(parseISO(bill.dueDate.toISOString()), finalPaymentDate) ||
        bill.status === "Overdue") &&
      actionType !== "rejectVerification"
    ) {
      const daysOverdue = differenceInDays(
        finalPaymentDate,
        parseISO(bill.dueDate.toISOString()),
      );
      if (daysOverdue > 0) {
    ...
    if (
      ...
      newStatus = isBefore(parseISO(bill.dueDate.toISOString()), today)
        ? "Overdue"
        : "Pending";
      ...
      if (newStatus === "Overdue") {
        const daysOverdueNow = differenceInDays(
          today,
          parseISO(bill.dueDate.toISOString()),
        );
        if (daysOverdueNow > 0) {
          ...
          billUpdateData.penaltyAmount =
            rejectedPenalty > 0 ? rejectedPenalty : null;
        } else {
