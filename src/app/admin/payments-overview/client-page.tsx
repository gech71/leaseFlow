...
export interface ClientPenaltyTier
  extends Omit<PenaltyTierPrisma, "id" | "feeValue"> {
  id?: string;
  feeValue: number;
}
...
export interface ClientSpaceForAgreement
  extends Omit<
    SpacePrismaOriginal,
    | "createdAt"
    | "updatedAt"
...
export interface ClientSpaceForPotentialRevenue
  extends Omit<
    SpacePrismaOriginal,
    | "createdAt"
    | "updatedAt"
...
export interface ClientAgreementForBill
  extends Omit<
...
export interface ClientUtilityBreakdownItem
  extends Omit<UtilityBreakdownItemPrismaOriginal, "id" | "billId"> {
  id?: string;
  billId?: string;
}
...
export function PaymentsOverviewClientPage({
  ...
  const totalUpcomingAmount = useMemo(
    () =>
      upcomingAndPendingBills.reduce((sum, bill) => sum + bill.totalAmount, 0),
        ...
        <Card className="shadow-sm bg-secondary/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-primary">
              {totalUpcomingAmount.toFixed(2)} Birr
            </div>
            <p className="text-xs text-muted-foreground">
              {upcomingAndPendingBills.length} transactions (incl. Overdue)
            </p>
          </CardContent>
        </Card>
        <Card className="shadow-sm bg-secondary/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {totalPaidSelectedPeriod.toFixed(2)} Birr
            </div>
            <p className="text-xs text-muted-foreground">
              {paidBillsInSelectedPeriod.length} transactions
            </p>
          </CardContent>
        </Card>
        <Card className="shadow-sm bg-secondary/50">
