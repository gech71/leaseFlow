    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Button onClick={exportToExcel} variant="outline" size="sm" disabled={filteredLogs.length === 0}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell text-right">Rent</TableHead>
                ...
                <TableBody>
                  {paginatedLogs.map((log) => (
                    <TableRow key={log.id}>
                      <TableCell className="font-medium whitespace-nowrap">{format(new Date(log.paymentDate), 'PP')}</TableCell>
                      <TableCell>
                        <div>{log.buildingName}</div>
                        <div className="text-xs text-muted-foreground">{log.spaceName}</div>
                      </TableCell>
                      <TableCell>{log.tenantName}</TableCell>
                      <TableCell className="text-right font-semibold">{log.totalAmount.toFixed(2)}</TableCell>
                      <TableCell className="hidden md:table-cell text-right text-sm">{log.rentAmount.toFixed(2)}</TableCell>
                      <TableCell className="hidden md:table-cell text-right text-sm">{log.utilityAmount.toFixed(2)}</TableCell>
                      <TableCell className="hidden lg:table-cell text-right text-sm text-destructive">{log.penaltyAmount.toFixed(2)}</TableCell>
                      <TableCell className="hidden lg:table-cell text-xs">{log.transactionId || 'N/A'}</TableCell>
                      <TableCell className="hidden xl:table-cell text-xs">{log.actorName}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
