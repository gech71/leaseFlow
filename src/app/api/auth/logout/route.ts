...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Error reading admin cookies for logout:", e);
  ...
  if (AUTH_API_BASE_URL && accessToken && refreshToken) {
    try {
      ...
      console.error("Error calling external logout API for admin:", error);
    }
  }
  ...
  try {
    ...
    cookieStore.set(REFRESH_TOKEN_KEY, '', {
      ...
      console.error("Error clearing admin cookies during logout:", cookieError.message);
