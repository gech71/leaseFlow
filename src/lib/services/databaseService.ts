...
export class DatabaseService {
  ...
  // --- ArifPayment ---
  async createArifPayment(
    data: Prisma.ArifPaymentCreateInput,
  ): Promise<ArifPayment> {
    return prisma.arifPayment.create({ data });
  }

  async getArifPaymentBySessionId(
    sessionId: string,
  ): Promise<ArifPayment | null> {
    return prisma.arifPayment.findUnique({ where: { sessionId } });
  }

  async updateArifPayment(
    id: string,
    data: Prisma.ArifPaymentUpdateInput,
  ): Promise<ArifPayment> {
